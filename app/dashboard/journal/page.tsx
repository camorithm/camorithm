...
{ id: 'ORD-006', date: 'Dec 09, 08:45 AM', symbol: 'BTCUSD', type: 'Buy', lots: 1.0, entry: 42000, exit: 42500, pl: 500, status: 'Win', tags: ['Swing'] },
];
...
return (
  <div className="max-w-7xl mx-auto pb-20">
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">Trading Journal</h1>
        <p className="text-slate-500 text-sm">Review your execution and analyze patterns.</p>
      </div>
      <div className="flex items-center gap-3">
        <button className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm shadow-blue-500/20">
          <Download size={16} />
          Export CSV
        </button>
      </div>
    ...
    {/* Toolbar & Filters */}
    <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-t-2xl p-4 border-b-0">
    ...
    {/* The Data Grid */}
    <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-b-2xl shadow-sm overflow-hidden">
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 dark:bg-white/[0.02] text-slate-500 font-semibold border-b border-slate-200 dark:border-white/5">
            <tr>
              ...
              <th className="px-6 py-4 cursor-pointer hover:text-slate-700 dark:hover:text-slate-300">
                <div className="flex items-center gap-1">Date <ChevronDown size={14} /></div>
              </th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-white/5">
            {trades.map((trade) => (
              <tr key={trade.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                ...
                <td className="px-6 py-4 text-right">
                  <button className="p-1 text-slate-400 hover:text-slate-700 dark:hover:text-white rounded hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
      ...
      {/* Pagination Footer */}
      <div className="px-6 py-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-between bg-slate-50/[0.5] dark:bg-white/[0.01]">
        <span className="text-xs text-slate-500">Showing <span className="font-bold text-slate-900 dark:text-white">1-6</span> of 42 trades</span>
        <div className="flex gap-2">
          <button className="px-3 py-1 text-xs font-medium text-slate-500 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded hover:bg-slate-50 dark:hover:bg-white/10 disabled:opacity-50" disabled>Previous</button>
          <button className="px-3 py-1 text-xs font-medium text-slate-700 dark:text-white bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded hover:bg-slate-50 dark:hover:bg-white/10">Next</button>
        </div>
      </div>
    </div>
  </div>
