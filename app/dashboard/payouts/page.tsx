...
import { CheckCircle2, AlertCircle, Download, Copy, Landmark, Bitcoin, FileText } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm">
          ...
          {method === 'crypto' ? (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <CryptoCard active={cryptoType === 'USDT'} onClick={() => setCryptoType('USDT')} symbol="USDT" network="ERC20 / TRC20" img="https://cryptologos.cc/logos/tether-tether-logo.png?v=025" />
                <CryptoCard active={cryptoType === 'BTC'} onClick={() => setCryptoType('BTC')} symbol="BTC" network="Bitcoin Network" img="https://cryptologos.cc/logos/bitcoin-btc-logo.png?v=025" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase">Destination Wallet Address</label>
                <div className="relative">
                  <input type="text" placeholder={`Enter your ${cryptoType} address`} className="w-full pl-4 pr-12 py-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-blue-500"><Copy size={16} /></button>
                </div>
                <p className="text-xs text-slate-400 mt-2 flex items-center gap-1"><AlertCircle size={12} /> Ensure you select the correct network.</p>
              </div>
            </div>
          ) : (
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
            ...
            <button className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-all shadow-lg shadow-blue-500/20 active:scale-95">Confirm Withdrawal</button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm">
          ...
          <button className="w-full flex items-center justify-center gap-2 py-2.5 border border-slate-200 dark:border-white/10 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"><Download size={16} /> Download PDF</button>
        </div>

        <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 font-bold text-sm text-slate-900 dark:text-white">Payout History</div>
          ...
          <button className="w-full py-3 text-xs font-medium text-slate-500 hover:text-blue-600 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">View All Transactions</button>
        </div>
      </div>
    </div>
  </div>
...
<div className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-white/[0.02] last:border-0">
  ...
  {done ? <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center"><CheckCircle2 size={12} className="text-white" /></div> : <div className="w-5 h-5 rounded-full border-2 border-slate-200 dark:border-slate-700"></div>}
</div>
);
const TabButton = ({ active, onClick, icon, label }: any) => (
<button onClick={onClick} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${ active ? 'bg-white dark:bg-[#0f1115] text-slate-900 dark:text-white shadow-sm ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }`}>{icon} {label}</button>
);
const CryptoCard = ({ active, onClick, symbol, network, img }: any) => (
<button onClick={onClick} className={`p-4 rounded-xl border text-left transition-all ${ active ? 'bg-blue-50 dark:bg-blue-500/10 border-blue-500 ring-1 ring-blue-500' : 'bg-slate-50 dark:bg-white/5 border-transparent hover:border-slate-200 dark:hover:border-white/10' }`}>
  <div className="flex items-center gap-2 mb-2"><img src={img} alt={symbol} className="w-6 h-6" /><span className="font-bold text-slate-900 dark:text-white">{symbol}</span></div>
...
<div className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
  <div className="flex items-center gap-3">
    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400"><FileText size={14} /></div>
    <div><div className="text-sm font-bold text-slate-900 dark:text-white">{amount}</div><div className="text-xs text-slate-500">{date}</div></div>
  </div>
  <div className="px-2 py-1 bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400 text-xs font-bold rounded">{status}</div>
</div>
);