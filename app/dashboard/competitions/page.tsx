...
import { Trophy, Timer, Users, Search } from 'lucide-react';
    ...
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <div className="flex items-center gap-2 text-blue-500 font-bold text-xs uppercase tracking-wider mb-2">
          <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span> Live Competition
        </div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">December Trading Cup</h1>
        <p className="text-slate-500 text-sm max-w-lg">Compete against 12,500 traders for a chance to win a $200k Challenge Account. Highest ROI wins.</p>
      </div>
      {/* Countdown Card */}
      <div className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl p-4 flex items-center gap-6 shadow-xl shadow-blue-900/20">
        <div><div className="text-[10px] uppercase opacity-70 font-bold mb-1">Time Remaining</div><div className="flex items-center gap-2 font-mono text-xl font-bold"><Timer size={20} className="text-blue-500" /> 14d : 06h : 42m</div></div>
        <div className="h-8 w-px bg-white/20 dark:bg-black/10"></div>
        <div><div className="text-[10px] uppercase opacity-70 font-bold mb-1">Prize Pool</div><div className="text-xl font-bold text-green-400 dark:text-green-600">$50,000</div></div>
      </div>
    </div>
        ...
        <div className="bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search trader..." className="pl-9 pr-4 py-1.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-lg text-xs focus:outline-none focus:border-blue-500 w-48" />
            </div>
          </div>
          {/* Table */}
            ...
            {leaderboard.slice(3).map((trader) => (
              <div key={trader.rank} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                ...
                <div className="col-span-4 flex items-center gap-3">
                  <img src={`https://flagcdn.com/${trader.country.toLowerCase()}.svg`} alt={trader.country} className="w-5 h-3.5 rounded-sm object-cover shadow-sm" />
                  <span className="font-medium text-slate-700 dark:text-slate-200">{trader.name}</span>
                </div>
                <div className="col-span-3 text-right font-bold text-green-500">+{trader.return}%</div>
                <div className="col-span-4 text-right font-mono text-slate-600 dark:text-slate-400">${trader.profit.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-6">
        <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 opacity-80 text-xs font-medium uppercase mb-4"><Users size={14} /> My Standing</div>
...
<div className={`relative overflow-hidden rounded-2xl p-6 border border-slate-200 dark:border-white/5 ${ isFirst ? 'bg-white dark:bg-[#0f1115] md:-mt-8 shadow-xl z-10' : 'bg-slate-50 dark:bg-white/[0.02]' }`}>
  <div className={`absolute top-4 right-4 w-8 h-8 rounded-full bg-gradient-to-br ${color} flex items-center justify-center text-white font-bold shadow-lg`}>{rank}</div>
  <div className="flex flex-col items-center text-center mt-4">
    ...
    <div className="text-3xl font-bold text-green-500 my-2">{data.return}%</div>
  </div>
</div>
);
const StatRow = ({ label, value }: any) => (
<div className="flex items-center justify-between py-2 border-b border-white/10 last:border-0"><span className="opacity-70 text-sm">{label}</span><span className="font-bold font-mono">{value}</span></div>
);